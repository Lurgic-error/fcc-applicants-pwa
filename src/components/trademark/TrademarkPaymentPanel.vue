<script setup>
/**
 * TrademarkPaymentPanel — GePG payment flow for trademark recordation.
 *
 * Flow:
 * 1. Applicant sees pre-filled payment details (amount from fee schedule, payer from profile)
 * 2. Applicant clicks "Generate Control Number" → system calls GePG API → returns control number
 * 3. Applicant pays externally (bank, mobile money) using the control number
 * 4. Applicant clicks "Verify Payment" → system checks GePG for confirmation
 * 5. Payment status updates: pending → control_number_issued → paid → verified
 */
