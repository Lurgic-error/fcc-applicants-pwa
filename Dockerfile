...
COPY fcc-applicants-pwa/package*.json ./
RUN npm install
COPY fcc-fims-infrastructure/shared /shared
COPY fcc-applicants-pwa/ .
RUN npm run build

FROM nginx:1.27-alpine
COPY fcc-applicants-pwa/nginx.conf /etc/nginx/conf.d/default.conf
